...
def prediction(img_path = "test.png",rect_path = "test_rect.txt"):

	net = PoseEstimationWithMobileNet()
	checkpoint = torch.load('weights/openpose.pth', map_location='cpu')
	load_state(net, checkpoint)
	rects = get_rect(net.cuda(), img_path, 512)
	np.savetxt(rect_path, np.array(rects), fmt='%d')

if __name__ == "__main__":
	prediction(img_path = "input/input.png",rect_path = "input/input_rect.txt")
